...
import { Database, HardDrive, Clock, CheckCircle, AlertTriangle, Play, Shield } from "lucide-react"
...
export default function BackupsPage() {
  const exitosos = demoBackups.filter((b) => b.status === "exitoso").length
  const fallidos = demoBackups.filter((b) => b.status === "fallido").length
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card><CardContent className="p-4 text-center"><Database className="size-5 mx-auto text-muted-foreground mb-1" /><p className="text-2xl font-bold">{demoBackups.length}</p><p className="text-xs text-muted-foreground">Backups Programados</p></CardContent></Card>
        ...
        <Card><CardContent className="p-4 text-center"><Shield className="size-5 mx-auto text-blue-500 mb-1" /><p className="text-2xl font-bold">{demoDRTests.length}</p><p className="text-xs text-muted-foreground">Pruebas DR (Q1)</p></CardContent></Card>
      </div>
