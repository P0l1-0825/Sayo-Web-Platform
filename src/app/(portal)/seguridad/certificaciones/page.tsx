...
import { ShieldCheck, Award, Calendar, CheckCircle, AlertTriangle, FileText } from "lucide-react"
...
export default function CertificacionesPage() {
  const vigentes = demoCerts.filter((c) => c.status === "vigente").length
  const enProceso = demoCerts.filter((c) => c.status === "en-proceso").length
  const porRenovar = demoCerts.filter((c) => c.status === "por-renovar").length
  const totalControles = demoCerts.reduce((s, c) => s + c.controles, 0)
  const completados = demoCerts.reduce((s, c) => s + c.controlesCompletos, 0)
