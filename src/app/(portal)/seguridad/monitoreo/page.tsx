...
import { Activity, Cpu, Clock, AlertTriangle, CheckCircle, Gauge, Wifi, Server } from "lucide-react"
...
export default function MonitoreoPage() {
  const healthy = demoServicios.filter((s) => s.status === "healthy").length
  const degraded = demoServicios.filter((s) => s.status === "degraded").length
  const down = demoServicios.filter((s) => s.status === "down").length
  const avgCpu = Math.round(demoServicios.filter((s) => s.status !== "down").reduce((s, sv) => s + sv.cpu, 0) / demoServicios.filter((s) => s.status !== "down").length)
